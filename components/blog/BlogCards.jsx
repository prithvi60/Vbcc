    ...
    <section className="block space-y-8">
      <div className="hidden lg:flex items-start gap-8">
        <div className="block space-y-5 lg:w-[55%] xl:w-[70%] text-warning sticky top-6">
          <div style={{backgroundImage: `url(${card.img})`}} className="bg-[url('/blog-2.png')] bg-cover bg-center w-full h-[520px] p-5">
            <div className="px-4 py-2.5 bg-white w-max rounded-md">{item.title}</div>
          </div>
        ...
        <div className="space-y-[50px] w-full lg:w-[45%] xl:w-[30%]">
          ...
          <div className="md:block space-y-5 text-warning w-full" key={idx}>
            <div className="relative w-full h-[330px] overflow-hidden">
                ...
                />
                <div className="px-4 py-2.5 absolute top-5 left-5 bg-white w-max rounded-md lg:hidden">{item.title}</div>
              </div>
            <h4 className="text-lg lg:text-[32px] font-Lora">
              {list.productTitile}
            </h4>
      ...
      {BlogCard.map((list,idx)=>(
          <div className="md:block space-y-5 text-warning w-full xl:last:col-span-3" key={idx}>
            <div className="relative w-full h-[330px] overflow-hidden">
                <Image
                  fill
                  src={list.img}
