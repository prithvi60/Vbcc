...
const MoreProducts = ({ data, type }) => {
    return (
        <section className="w-full h-full space-y-10 overflow-hidden bg-primary padding font-Montserrat">
            <h4 className="relative w-full text-xl font-bold tracking-wide text-white capitalize md:text-3xl xl:text-4xl after:absolute after:h-1 after:w-1/4 after:-bottom-4 after:left-0 after:bg-info">{`More ${type === "Ball_Mills" ? "Ball Mills" : type} ${type === "dental" || type === "laboratory" ? "Furnaces" : ""}`}</h4>
                ...
                {data.map((list, idx) => (
                    ...
                    >
                        ...
                        <div className="space-y-8 w-max">
                            ...
                            <button>
                                <Link
                                    href={`/categories/material_processing_equipment/${type}/${list.productName.replace(/\s/g, "_")}`}
